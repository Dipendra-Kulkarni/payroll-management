...
const taxDocuments = [
  { id: 'w2-2024', name: 'W-2 Tax Form 2024', year: '2024', available: false, availableDate: '2025-01-31' },
  { id: 'w2-2023', name: 'W-2 Tax Form 2023', year: '2023', available: true, downloadUrl: '#' },
  { id: '1099-2023', name: '1099 Form 2023', year: '2023', available: true, downloadUrl: '#' }
];

const benefitsEnrollment = [
  { id: 'health', name: 'Health Insurance', enrolled: true, cost: 350, coverage: 'Employee + Family' },
  { id: 'dental', name: 'Dental Insurance', enrolled: true, cost: 45, coverage: 'Employee + Family' },
  { id: 'vision', name: 'Vision Insurance', enrolled: false, cost: 25, coverage: 'Not Enrolled' },
  { id: '401k', name: '401(k) Retirement', enrolled: true, cost: 195.38, coverage: '6% of salary' },
  { id: 'life', name: 'Life Insurance', enrolled: true, cost: 12, coverage: '$50,000' }
];
...
const samplePayStubData = {
  ...
  deductions: {
    ...
    dentalInsurance: 45.00,
    retirement401k: 195.38,
    totalDeductions: 892.50
  },
  netPay: 2376.73,
  ytdTotals: {
    grossPay: 85000,
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <Card className="border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5">
            ...
            <CardContent>
              <div className="grid gap-6 md:grid-cols-3">
                <div>
                  <p className="text-sm text-muted-foreground">Estimated Gross Pay</p>
                  <p className="text-3xl font-bold text-primary">$3,269</p>
                  <p className="text-sm text-muted-foreground">Based on 80 hours</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Estimated Deductions</p>
                  <p className="text-2xl font-bold">$892</p>
                  <p className="text-sm text-muted-foreground">Taxes & Benefits</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Estimated Net Pay</p>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">401(k) Balance</CardTitle>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
              ...
              <CardContent>
                <div className="space-y-3">
                  {payStubs.slice(0, 3).map((stub) => (
                    <div key={stub.id} className="flex items-center justify-between p-3 border rounded">
                      <div>
                      ...
                      <div className="text-right">
                        <p className="font-medium">${stub.netPay.toLocaleString()}</p>
                        <div className="flex items-center gap-2">
                          <Badge variant={stub.status === 'current' ? 'secondary' : 'outline'} className="text-xs">
                            {stub.status}
                          </Badge>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {benefitsEnrollment.filter(b => b.enrolled).map((benefit) => (
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {payStubs.map((stub) => (
                  <div key={stub.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-4">
                      <FileText className="h-8 w-8 text-primary" />
                      <div>
                        <h3 className="font-medium">{stub.period}</h3>
                        <p className="text-sm text-muted-foreground">
                          Pay Date: {new Date(stub.payDate).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      <div className="flex items-center gap-4 mb-2">
                        ...
                        <Badge variant={stub.status === 'current' ? 'secondary' : 'outline'}>
                          {stub.status}
                        </Badge>
                      </div>
                      <div className="flex gap-2">
                        <Dialog>
                          ...
                          <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                              <DialogTitle>Pay Stub - {stub.period}</DialogTitle>
                              <DialogDescription>
                  ...
                  <div key={benefit.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center gap-4">
                      <div className="p-2 bg-muted rounded">
                        {benefit.id === 'health' && <Building className="h-5 w-5" />}
                        {benefit.id === 'dental' && <Building className="h-5 w-5" />}
                        {benefit.id === 'vision' && <Eye className="h-5 w-5" />}
                        {benefit.id === '401k' && <PiggyBank className="h-5 w-5" />}
                        {benefit.id === 'life' && <CheckCircle className="h-5 w-5" />}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className="font-medium">
                          {benefit.enrolled ? `$${benefit.cost}/month` : 'Available'}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {benefit.enrolled ? 'Current cost' : 'Monthly cost'}
                        </p>
                      </div>
                      <Badge variant={benefit.enrolled ? 'default' : 'secondary'} className={
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Next enrollment period information</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="text-center space-y-4">
                  <div className="p-4 bg-primary/5 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground">November 1-30, 2025</p>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    You can make changes to your benefit elections during the open enrollment period.
                  </p>
        ...
        <TabsContent value="taxes" className="space-y-6">
                      ...
                      {doc.available ? (
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant="secondary">
                            Available {new Date(doc.availableDate).toLocaleDateString()}
                          </Badge>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Important tax-related information</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="p-4 border rounded-lg bg-blue-50">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="font-medium text-blue-900">W-2 Forms Available January 31st</h3>
                      <p className="text-sm text-blue-700">
                        Your 2024 W-2 tax form will be available for download by January 31, 2025.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 border rounded">
                    <p className="text-sm text-muted-foreground">YTD Federal Tax</p>
                    <p className="text-lg font-bold">$17,010</p>
                  </div>
                  <div className="p-3 border rounded">
                    <p className="text-sm text-muted-foreground">YTD State Tax</p>
                    <p className="text-lg font-bold">$4,250</p>
                  </div>
                  <div className="p-3 border rounded">
                    <p className="text-sm text-muted-foreground">YTD Social Security</p>
                    <p className="text-lg font-bold">$5,270</p>
                  </div>
                  <div className="p-3 border rounded">
                    <p className="text-sm text-muted-foreground">YTD Medicare</p>
