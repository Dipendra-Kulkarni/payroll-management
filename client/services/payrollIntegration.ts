...
export class PayrollIntegrationService {
  ...
  static calculateEmployeePayroll(
    ...
    const regularHours = timeEntries.reduce((total, entry) => total + entry.regularHours, 0);
    ...
    if (employee.payType === 'hourly') {
      ...
      overtimePay = overtimeHours * (employee.overtimeRate || employee.payRate * 1.5);
    } else {
    ...
    const benefitDeductions = Object.values(employee.benefits).reduce((sum, amount) => sum + amount, 0);
   ...
   */
  private static calculateFederalTax(grossPay: number, employee: Employee): number {
   ...
   */
  private static calculateStateTax(grossPay: number, employee: Employee): number {
   ...
   */
  static validateTimeEntries(timeEntries: TimeEntry[]): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  static generatePayrollSummary(
    ...
    const totalTaxes = calculations.reduce((sum, calc) => 
      sum + calc.federalTax + calc.stateTax + calc.socialSecurityTax + calc.medicareTax, 0
    );
    const totalBenefits = calculations.reduce((sum, calc) => sum + calc.benefitDeductions, 0);
  ...
  static checkCompliance(
    ...
    const effectiveHourlyRate = totalHours > 0 ? calculation.grossPay / totalHours : 0;
    const minimumWage = 15.00; // Example minimum wage
    
    if (effectiveHourlyRate < minimumWage) {
      violations.push(`Pay rate below minimum wage: $${effectiveHourlyRate.toFixed(2)}`);
    }
  ...
  private static exportToCSV(calculations: PayrollCalculation[]): string {
    const headers = [
      'Employee ID', 'Period ID', 'Regular Hours', 'Overtime Hours',
      'Regular Pay', 'Overtime Pay', 'Gross Pay', 'Federal Tax',
      'State Tax', 'Social Security', 'Medicare', 'Benefits',
